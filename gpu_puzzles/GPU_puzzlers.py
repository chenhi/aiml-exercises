...
def conv_test(cuda):
    def call(out, a, b, a_size, b_size) -> None:
        ...
        # First 4 threads read b
        if local_i < b_size:
          b_shared[local_i] = b[local_i]
        # Last 4 threads read extra "next" entries of a
        elif local_i >= MAX_CONV and i + TPB - MAX_CONV < a_size:
          a_shared[local_i + TPB - MAX_CONV] = a[i + TPB - MAX_CONV]
        ...
        for j in range(0, b_size):
          if i + j < a_size:
            total += a_shared[local_i + j] * b_shared[j]
...
def sum_test(cuda):
    def call(out, a, size: int) -> None:
        ...
        cache[local_i] = a[i] if i < size else 0.
        cuda.syncthreads()
        
        modulus = 2
        while modulus <= TPB:     # Assume it's a power of 2
          if local_i % modulus == 0:
            cache[local_i] = cache[local_i] + cache[local_i + modulus//2]          # Values only in evens
          modulus *= 2
          cuda.syncthreads()
        if local_i == 0:
...
def axis_sum_test(cuda):
    def call(out, a, size: int) -> None:
        ...
        # FILL ME IN (roughly 12 lines)
        # Set-up: number of blocks = one for each column
        # Size has to be less than TPB?  I guess that's the set-up, no blocks to handle the extra cases
        cache[local_i] = a[batch, i] if i < size else 0.
        cuda.syncthreads()
...
def mm_oneblock_test(cuda):
    def call(out, a, b, size: int) -> None:
        ...
        # Initialize the out (don't do this in the loop...)
        out_shared = cuda.shared.array((TPB, TPB), numba.float32)      # Will reduce the number of global writes?
        out_shared[local_i, local_j] = 0.

        for block in range(0, (size + TPB - 1)//TPB):
          # These variables correspond to the (local i, local j) position in the moving block
          block_i = local_i + TPB * block
          block_j = local_j + TPB * block

          # Copy the block at position (i's block, moving block)
          a_shared[local_i, local_j] = a[i, block_j] if i < size and block_j < size else 0.
          # Copy the block at position (moving block, j's block)
          b_shared[local_i, local_j] = b[block_i, j] if block_i < size and j < size else 0.

          cuda.syncthreads()

          sum = 0.
          for k in range(0, TPB):
            sum += a_shared[local_i, k] * b_shared[k, local_j]            # Local memory is faster than shared memory?
          out_shared[local_i, local_j] = out_shared[local_i, local_j] + sum
...
#problem.show(sparse=True)              # Had to disable this
...
inp1 = np.arange(SIZE * SIZE).reshape((SIZE, SIZE))
inp2 = np.arange(SIZE * SIZE).reshape((SIZE, SIZE)).T
#inp1 = np.ones((SIZE, SIZE))
#inp2 = np.ones((SIZE, SIZE))
#inp2 = np.eye(SIZE)
