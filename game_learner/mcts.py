...
class MCTSQFunction(PrototypeQFunction):
    ...
    def ucb_get(self, state: torch.Tensor, heuristic_parameter= 0.):
        ...
        return q.nan_to_num(0) + p * (heuristic_parameter * math.sqrt(1 + n_tot) / (1 + n))         # TODO q is a value so can be negative... ???? also do we want "Get" to return N or ...?
    ...
    def get(self, state: torch.Tensor, action = None, heuristic_parameter= 1.):
        q, n_tot, n = torch.tensor([], device=self.device), torch.tensor([], device=self.device), torch.tensor([], device=self.device)
        for i in range(state.size(0)):
            statehash = self.mdp.state_to_hashable(state)
            if statehash in self.q:
                q = torch.cat([q, self.q[statehash][None]])
        ...
        # Exploration vs. exploitation: the second term dominates when unexplored, then the first term dominates when more explored
        if action == None:
            return q.nan_to_num(0) + self.mdp.masked_softmax(self.h(state)) * (heuristic_parameter * math.sqrt(1 + n_tot) / (1 + n))         # TODO q is a value so can be negative... ???? also do we want "Get" to return N or ...?
        else:
            return (action * (q.nan_to_num(0) + self.mdp.masked_softmax(self.h(state)) * (heuristic_parameter * math.sqrt(n_tot) / (1 + n)))).sum(tuple(range(1,action.dim())))

    def policy(self, state: torch.Tensor, heuristic_parameter = 0., stochastic=False, valid_filter=True):
        if stochastic:
    ...
    def search(self, state, heuristic: nn.Module, num: int, ucb_parameter, temperature=1.0, max_depth=1000):
        ...
        for i in range(num):
            ...
            if self.mdp.is_terminal(s).item() == False:
                ...
                self.q.p[self.mdp.state_to_hashable(s)] = self.mdp.masked_softmax(heuristic(s))
            ...
            while self.mdp.is_terminal(s).item() == False:
                if depth >= max_depth:
                    ...
                    print(f"Heuristic: {heuristic(s)}")
                    print(f"Probabilities: {torch.exp(heuristic(s) + self.mdp.neginf_kill_actions(s))}")
                    break
                # Exponential means un-normalized softmax
                action = self.mdp.get_random_action_weighted(self.mdp.masked_softmax(heuristic(s)))      # TODO nan to num a hack -- when heuristic values too large get a bunch of infinities which function can't handle 
