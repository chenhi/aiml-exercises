...
class C4TensorMDP(MDP):
    def __init__(self):
        # State tensor (num_batches, which_player, player_channel, board_width, board_height)
        ...
        self.p0 = torch.tensor([1,0], dtype=float)
        self.p1 = torch.tensor([0,1], dtype=float)
    ...
    def get_initial_state(self, start_player = 0, batch_size=1):
        if start_player != 0 and start_player != 1:
            start_player = random.choice((0,1))
        batch_factor = torch.ones((batch_size, ), dtype=float)
        if start_player == 0:
            return torch.tensordot(batch_factor, torch.tensordot(self.p0, torch.zeros((2,7,6), dtype=float), 0), 0)
        else:
            return torch.tensordot(batch_factor, torch.tensordot(self.p1, torch.zeros((2,7,6), dtype=float), 0), 0)
